	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		if(reCaptchaTestResult) {
			...
			if(validation) {
				...
				User user = new User(name,email,password,contact,Long.toString(activationCode));
				
				user.signUp();
				ServletContext context = request.getServletContext();
				String parentPath = context.getRealPath("/WEB-INF/uploads/users");
				File file = new File(parentPath, user.getEmail());
				
				file.mkdir();
				
				String subject = "Foody :: Account Activation Mail";
				String message = EmailMessages.getUserAccountActivationMail(name, email, activationCode);
